...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/*:Осмотр_рефлексотерапевта">
		<html>
			<head>
				...
				<meta spec_version="1.1.0"/>
				<meta xslt_version="1.0.0"/>
				<title>XSLT</title>
				<style>
					
					.xslt_jira_ticket_name_s * {
					...
					.xslt_jira_ticket_name_s .xslt-title{
					font-weight: bold;
					color: #333333;
					text-align: left;
					}
					
					.xslt_jira_ticket_name_s .xslt-item {
					margin: 20px 0;
					}
					
					.xslt_jira_ticket_name_s .xslt-item__content .xslt-item:first-child {
					margin-top: 0;
					}
					.xslt_jira_ticket_name_s .xslt-subheader {
					text-align: left;
					color: #757575;
					font-weight: normal;
					}
					
					.xslt_jira_ticket_name_s p, h2 {
					margin: 0;
					}
			...
			<body>
				<div class="xslt_jira_ticket_name_s">
					<xsl:if
						test="true()">
						<xsl:call-template name="section">
							...
							<xsl:with-param name="content">
								...
								<xsl:call-template name="sub_section">
									<xsl:with-param name="title">Тестовый заголовок 2 уровня</xsl:with-param>
									<xsl:with-param name="content">
										<p>
											Какой-то текст
										</p>
  ...
  <!-- ******************************************************************************************************************************* -->
	<xsl:template name="section">
		...
		<div class="xslt-item">
			<xsl:if test="$title !=''">
				<h2 class="xslt-title"><xsl:value-of select="$title"/></h2>
			</xsl:if>
			<xsl:if test="$content !=''">
				<div class="xslt-item__content">
	...
	<xsl:template name="sub_section">
		...
		<div class="xslt-item">
			<xsl:if test="$title !=''">
				<span class="xslt-subheader"><xsl:value-of select="$title"/></span>
			</xsl:if>
			<xsl:if test="$content !=''">
				<div class="xslt-item__content">
					<xsl:copy-of select="$content"/>
  ...
  <xsl:template name="date-format">
    ...
    <xsl:variable name="date"
      select="xs:date(substring($dateString, 1, 10))"/>
